 ...
 * Heart class.
 */
class Heart(centerX: Float, centerY: Float, val width: Float, val height: Float) : BaseType() {

    companion object {
        const val POINT_NUMBER = 3
    }

    var centerTop: PointF = PointF()
    var leftTop: Array<PointF> = Array(POINT_NUMBER, { PointF() })
    var leftBottom: Array<PointF> = Array(POINT_NUMBER, { PointF() })
    var rightBottom: Array<PointF> = Array(POINT_NUMBER, { PointF() })
    var rightTop: Array<PointF> = Array(POINT_NUMBER, { PointF() })
    ...
    private fun initPoint(centerX: Float, centerY: Float, width: Float, height: Float) {
        centerTop.x = centerX
